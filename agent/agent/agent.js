...
function fiscalSanitize(str) {
  // scoate diacritice + caractere ciudate
  return String(str || "")
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")     // scoate diacritice combinate
    .replace(/[^\x20-\x7E]/g, " ")       // lasă doar ASCII printabil (32..126)
    .replace(/\s+/g, " ")
    .trim();
}

function splitReceiptLines(desc) {
  const raw = String(desc || "").replace(/\r\n/g, "\n");
  const parts = raw.split("\n").map(s => fiscalSanitize(s)).filter(Boolean);
  if (parts.length === 0) return ["BILET CALATORIE"];
  return parts;
}
...
async function callFiscal(path, body, devLabel = FISCAL_DEVICE_ID) {
  const url = `${FISCAL_BASE_URL}${path}?dev=${encodeURIComponent(devLabel)}`;
...
async function callFiscalGet(path, devLabel = FISCAL_DEVICE_ID) {
  const url = `${FISCAL_BASE_URL}${path}?dev=${encodeURIComponent(devLabel)}`;
  console.log("[AGENT] [FISCAL] GET", url);
  ...
  console.log("[AGENT] [FISCAL] GET response status=", res.status, "body=", data);
...
async function handleJob(job) {
  ...
  const receiptDesc = String(payload.description || "BILET");
  ...
  if (job.job_type === "cash_receipt_only") {
    ...
    try {
      ...
      const lines = splitReceiptLines(receiptDesc);

      const firstLine = lines[0] || "BILET CALATORIE";
      const extraLines = lines.slice(1);

      console.log("[AGENT] [FLOW] fiscal/sale (cash) name=", firstLine);
      ...
      // ✅ Tipărim detaliile (rând cu rând) — SUPER SIGUR
      let textPrintOk = true;
      let textPrintError = null;

      for (const line of extraLines) {
        try {
          console.log("[AGENT] [FLOW] fiscal/text", line);
          await callFiscal("/fiscal/text", { text: line }, devLabel);
        } catch (e) {
          textPrintOk = false;
          textPrintError = String(e?.message || e);
          console.error("[AGENT] [FLOW] fiscal/text FAILED:", textPrintError);
          break;
        }
      }

      // ✅ Dacă textul a picat (ex NO_PAPER), NU mai facem pay/close
      if (!textPrintOk) {
        report.success = false;
        report.fiscal_ok = false;
        report.result.fiscal.ok = false;
        report.error_message =
          "NO_PAPER / TEXT_FAILED. Casa asteapta hartie si poate continua automat. " +
          (textPrintError || "");
      ...
      await callFiscal("/fiscal/close", {}, devLabel);
  ...
  if (job.job_type === "card_and_receipt") {
    ...
    try {
      ...
      // ✅ Împărțim descrierea în linii (la fel ca la cash_receipt_only)
      const lines = splitReceiptLines(receiptDesc);

      const firstLine = lines[0] || "BILET CALATORIE";
      const extraLines = lines.slice(1);

      console.log("[AGENT] [FLOW] fiscal/sale (card) name=", firstLine);
      await callFiscal("/fiscal/sale", {
        name: firstLine,
        tax: "1",
        price: amount,
      ...
      if (!textPrintOk) {
        ...
        // mesaj business: POS ok, bon incomplet
        report.error_message =
          "Plata reusita la POS. NO_PAPER / TEXT_FAILED. Casa asteapta hartie si poate continua automat. " +
          (textPrintError || "");
  ...
  if (job.job_type === "card_refund") {
    ...
    try {
      ...
      const posStan = payload.pos_stan || null;
      const posBody = {
        amount: amount.toFixed(2),
        currency: payload.currency || "RON",
        stan: posStan,
      ...
      if (!posStan || !/^\d{6}$/.test(String(posStan).trim())) {
        throw new Error("Lipseste STAN (A109) pentru refund/void.");
      }
  ...
  if (job.job_type === "retry_receipt") {
    ...
    try {
      ...
      // ✅ Împărțim descrierea în linii (ca la cash/card)
      const lines = splitReceiptLines(receiptDesc);
      const firstLine = lines[0] || "BILET CALATORIE";
      const extraLines = lines.slice(1);

      console.log("[AGENT] [RETRY] fiscal/sale name=", firstLine);
      await callFiscal(
        "/fiscal/sale",
        {
          name: firstLine,
      ...
      for (const line of extraLines) {
        try {
          console.log("[AGENT] [RETRY] fiscal/text", line);
          await callFiscal("/fiscal/text", { text: line }, devLabel);
        } catch (e) {
          textPrintOk = false;
          textPrintError = String(e?.message || e);
          console.error("[AGENT] [RETRY] fiscal/text FAILED:", textPrintError);
          break;
        }
      }

      // ✅ IMPORTANT: dacă textul a picat (ex NO_PAPER), NU mai facem pay/close
      if (!textPrintOk) {
        report.success = false;
        report.fiscal_ok = false;
        report.result.fiscal.ok = false;
      ...
      // nu suprascriem mesajul “NO_PAPER / TEXT_FAILED...” dacă l-am setat deja
      if (!report.error_message) {
    ...
    try {
      ...
      console.log(`[AGENT] Report trimis pentru job #${job.id} (retry_receipt)`);
    } catch (err) {
      console.error(
        `[AGENT] Eroare la trimiterea report-ului pentru job #${job.id}:`,
        err
...
function startAgentSocket() {
  ...
  agentSocket.on("connect_error", (err) => {
    ...
    const serializedError = serializeSocketError(err);
    console.error(
      "[AGENT][SOCKET] Eroare conectare:",
...
process.on("SIGINT", () => {
  ...
  try {
    if (agentSocket) {
      console.log("[AGENT] Inchid socket...");
      agentSocket.disconnect();
      if (typeof agentSocket.close === "function") agentSocket.close();
    }
  } catch (e) {
    console.log("[AGENT] Eroare la inchiderea socket-ului:", e?.message || e);
  }

  // daca mai ramane ceva deschis, iesim fortat
