...
// HTTP port pentru POS bridge
// (compatibil cu .env-ul tău: POS_PORT=9100)
const HTTP_PORT = Number(process.env.POS_PORT || process.env.POS_HTTP_PORT || 9100);

// Porturi COM pentru cele doua POS-uri
// (compatibil cu .env-ul tău: POS_DEVICE_A / POS_DEVICE_B)
const POS_DEVS = {
  A: process.env.POS_DEVICE_A || process.env.POS_DEV_A || "COM12",
  B: process.env.POS_DEVICE_B || process.env.POS_DEV_B || "COM12",
};
...
const POS_EXPECTED = {
  A: (process.env.POS_A_EXPECTED_ID || "").toUpperCase(),
  B: (process.env.POS_B_EXPECTED_ID || "").toUpperCase(),
};

const POS_MATCH = {
  PRISCOM: process.env.PRISCOM_POS_MATCH || "",
  AUTODIMAS: process.env.AUTODIMAS_POS_MATCH || "",
};

const POS_BLOCK_ON_MISMATCH =
  String(process.env.POS_BLOCK_ON_MISMATCH || "0") === "1";

const POS_IDENTITY = {
  A: { ok: null, expected: POS_EXPECTED.A, actual: null },
  B: { ok: null, expected: POS_EXPECTED.B, actual: null },
};

// ───────────── Log doar la schimbare (anti-spam) ───────────────

const LAST_LOG_STATE = {
  A: Object.create(null),
  B: Object.create(null),
};

function logOnChange(dev, bucket, key, logFn, msg) {
  const prev = LAST_LOG_STATE?.[dev]?.[bucket];
  if (prev === key) return;
  LAST_LOG_STATE[dev][bucket] = key;
  logFn(msg);
}
...
function buildGetInfoTLV() {
  return Buffer.concat([
    tlv(0xA000, Buffer.from([0x01])), // Get POS information
  ]);
}
...
function buildRefundTLV(amountLei, stan6, extraTags = []) {
  const amountStr = Number(amountLei).toFixed(2).replace(".", "");
  const amount12 = amountStr.padStart(12, "0");

  const stan = String(stan6 || "").trim().padStart(6, "0").slice(0, 6);
  if (!/^\d{6}$/.test(stan)) {
    throw new Error("STAN invalid/missing pentru VOID (A006).");
  }
  const tlvs = [
    tlv(0xA000, Buffer.from([0x06])), // ✅ 06 – Perform void
    tlv(0xA006, ascii(stan)),         // ✅ STAN (6)
    tlv(0xA001, ascii(amount12)),     // ✅ Amount (12)
  ];
...
function getCommandCodeFromRequestTLV(tlvReq) {
  try {
    // TLV: [tagHi, tagLo, len, ...data]
    if (!Buffer.isBuffer(tlvReq) || tlvReq.length < 4) return null;

    const tag = (tlvReq[0] << 8) | tlvReq[1];
    const len = tlvReq[2];

    if (tag !== 0xA000) return null;
    if (len !== 0x01) return null;
    if (tlvReq.length < 3 + len) return null;

    return tlvReq[3]; // command byte (ex: 0x01, 0x02, 0x03...)
  } catch {
    return null;
  }
}
...
async function sendPosCommand(dev, tlvBuf) {
  ...
  const cmd = getCommandCodeFromRequestTLV(tlvBuf);
  const isGetInfo = cmd === 0x01;
  ...
  return new Promise((resolve, reject) => {
    ...
    port.on("data", (chunk) => {
      ...
      for (const b of chunk) {
        if (state === "waitAckEnq") {
          if (b === ACK) {
            if (!isGetInfo) console.log("POS ACK la ENQ → trimit frame TLV");
          ...
          if (fullFrameLen && respBuf.length >= fullFrameLen) {
            ...
            // Pentru Get Info (A000=01) NU există A107 în răspuns.
            // OK = true dacă A100=00.
            if (cmd === 0x01) {
              var ok = resTagHex === "00";
            } else {
              // Pentru comenzi financiare / host-based: A100=00 și A107=00/Y1/Y3
              var ok =
                resTagHex === "00" &&
            ...
            if (isGetInfo) {
              // log doar când se schimbă ceva la status GetInfo
              const a102 = bufToAscii(tags?.A102 || Buffer.from("")).trim();
              const statusKey = `ok=${ok}|a100=${resTagHex || ""}|a102=${a102}`;
              ...
              logOnChange(
                ...
                statusKey,
                console.log,
                `ℹ️ POS ${dev} GetInfo changed: ok=${ok} a100=${resTagHex || "(lipsa)"} a102="${a102}"`
              );
            } else {
              // pentru comenzi reale, păstrăm debug complet
    ...
    port.open((err) => {
      if (err) {
        ...
        logOnChange(
          ...
          `${portName}|${msg}`,
          console.error,
          `Nu pot deschide portul POS ${dev} (${portName}): ${msg}`
        );
...
function classifyPos(tags) {
  // A102 = Hardware version / serial number (ASCII) – identificăm POS-ul doar din A102
  const a102Raw = bufToAscii(tags?.A102 || Buffer.from("")).trim();
  const a102 = a102Raw.toUpperCase();

  if (!a102) return null;

  for (const [name, ruleRaw] of Object.entries(POS_MATCH)) {
    const rule = String(ruleRaw || "").trim();
    if (!rule) continue;

    // acceptăm formatul tău din .env: TEXT:xxxx
    const [typeRaw, ...rest] = rule.split(":");
    const type = String(typeRaw || "").trim().toUpperCase();
    const value = rest.join(":").trim().toUpperCase(); // în caz că serialul conține ':'

    if (!value) continue;

    // cerința ta: NU merchant id, doar A102
    if (type === "TEXT") {
      if (a102.includes(value)) return name;
    }
  }
...
async function refreshPosIdentity() {
  for (const dev of ["A", "B"]) {
    try {
      const result = await posGetInfo(dev);

      // Actual = PRISCOM / AUTODIMAS / null
      const actual = classifyPos(result.tags);

      const expected = (POS_EXPECTED[dev] || "").toUpperCase();

      // ok = true doar dacă:
      //  - am reușit să identificăm (actual != null)
      //  - și dacă ai setat expected în .env, actual trebuie să fie exact expected
      const ok =
        !!actual &&
        (!expected || actual === expected);

      const next = { ok, expected, actual };

      const prev = POS_IDENTITY[dev];
      const changed =
        !prev ||
        prev.ok !== next.ok ||
      ...
      POS_IDENTITY[dev] = next;

      if (changed) {
        if (next.ok) {
          console.log(`✅ POS ${dev}: identity OK -> ${next.actual} (A102 match)`);
        } else {
          console.error(
            `❌ POS ${dev}: identity MISMATCH/UNKNOWN. expected=${next.expected || "(not set)"} actual=${next.actual || "(unidentified)"}`
          );
        }
      }
    } catch (e) {
      // IMPORTANT: dacă nu pot citi info, marchez explicit ca unknown (ok=false),
      // ca să poți bloca tranzacțiile când POS_BLOCK_ON_MISMATCH=1.
      const expected = (POS_EXPECTED[dev] || "").toUpperCase();
      const next = { ok: false, expected, actual: null };
      ...
      if (changed) {
        console.error(`❌ POS ${dev}: identity read failed -> ${String(e?.message || e)}`);
      }
    }
  }
}
...
setTimeout(() => {
  refreshPosIdentity();
  setInterval(refreshPosIdentity, 15000);
}, 2000);
...
async function posRefund(dev, amountLei, stan, extraTags) {
  const tlvReq = buildRefundTLV(amountLei, stan, extraTags);
  return sendPosCommand(dev, tlvReq);
}


// ================= IDENTITY BLOCK HELPER =================

function enforcePosIdentityOrBlock(dev, res) {
  if (!POS_BLOCK_ON_MISMATCH) return true;

  const state = POS_IDENTITY[dev];

  // Blocare totală dacă:
  // - nu am state încă
  // - sau ok !== true (include false sau null)
  if (!state || state.ok !== true) {
    return res.status(409).json({
      ok: false,
      error: "POS_DEVICE_MISMATCH",
      message: `POS ${dev} nu corespunde configuratiei sau nu poate fi identificat. expected=${state?.expected || "(not set)"} actual=${state?.actual || "(unidentified)"}`,
    });
  }
    ...
    pos: [
      {
        ...
        path: POS_DEVS.A,
        baud: POS_BAUD,
        identity: POS_IDENTITY.A,
      },
      {
        id: "B",
        path: POS_DEVS.B,
        baud: POS_BAUD,
        identity: POS_IDENTITY.B,
      },
    ],
    expected_map: POS_EXPECTED,
    block_on_mismatch: POS_BLOCK_ON_MISMATCH,
  };
...
app.get("/pos/info", async (req, res) => {
  ...
  try {
    const result = await posGetInfo(dev);
    res.json({
      ok: String(result.errorCode || "").toUpperCase() === "00",
...
app.post("/pos/sale", async (req, res) => {
  ...
  if (!enforcePosIdentityOrBlock(dev, res)) return;
...
app.post("/pos/refund", async (req, res) => {
  ...
  if (!stan || !/^\d{6}$/.test(String(stan).trim())) {
    return res.status(400).json({ ok: false, error: "STAN_REQUIRED_OR_INVALID" });
  }
  console.log(`\n=== /pos/refund dev=${dev} amount=${amount.toFixed(2)} ===`);

  try {
    const result = await posRefund(dev, amount, stan, extraTags);
    if (!result.ok) {
      const message = mapPosDeclineMessageFromTags(
        result.tags,
        result.errorCode,
...
// Logheaza doar cand portul setat in .env nu e gasit / reapare.
// Nu mai spameaza stdout la fiecare pornire / repetat.

let _lastPosPresence = { A: null, B: null };

function _normalizePortNameForCompare(s) {
  // SerialPort.list() intoarce de obicei "COM12" (fara \\.\)
  return String(s || "").trim().toUpperCase().replace(/^\\\\\.\\/, "");
}

async function monitorPosPortsOnce() {
  try {
    const ports = await SerialPort.list();
    const present = new Set(ports.map(p => _normalizePortNameForCompare(p.path)));

    for (const dev of ["A", "B"]) {
      const wantedRaw = POS_DEVS[dev];
      const wanted = _normalizePortNameForCompare(wantedRaw);
      const isPresent = present.has(wanted);

      // log doar cand se schimba starea (prima data sau schimbare)
      if (_lastPosPresence[dev] === null) {
        // prima verificare: logam doar daca lipseste (ca sa nu umplem log-ul)
        if (!isPresent) {
          console.error(`❌ POS ${dev}: portul din .env NU este gasit: ${wantedRaw}`);
        }
      } else if (_lastPosPresence[dev] !== isPresent) {
        if (isPresent) {
          console.log(`✅ POS ${dev}: port gasit din nou: ${wantedRaw}`);
        } else {
          console.error(`❌ POS ${dev}: port DISPARUT / nu este gasit: ${wantedRaw}`);
        }
      }

      _lastPosPresence[dev] = isPresent;
    }
  } catch (err) {
    // nu spamam: doar o singura data daca apare si apoi doar la schimbare
    const msg = String(err?.message || err);
    console.error("Eroare monitorPosPortsOnce:", msg);
  }
}
...
setTimeout(() => {
  monitorPosPortsOnce();
  setInterval(monitorPosPortsOnce, 5000);
}, 1500);
