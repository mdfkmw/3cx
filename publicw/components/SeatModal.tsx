...
return () => {
  ...
  }, [contact.name, contactNameTouched, hasAccountContact, isOpen])
  ...
  const handleSubmit = async () => {
    ...
    setSubmitError(null)
    try {
      const ownerName = trimmedName || accountContact.name || ''
      await onConfirm({
        seats: selectedSeats,
                    ...
                    <div className="space-y-3">
                      <div>
                        ...
                        <p className="text-sm text-white/70">
                          {hasAccountContact
                            ? 'Numele titularului se folosește pentru toate locurile. Selectează doar reducerea potrivită pentru fiecare loc.'
                            : 'Numele introdus la contact va fi folosit pentru toate locurile. Selectează reducerea potrivită pentru fiecare loc.'}
                        </p>
                      </div>
                      ...
                      {passengerList.length > 0 && (
                        <div className="space-y-3">
                          {passengerList.map((passenger, index) => {
                            ...
                            const displayName = isPrimaryPassenger ? accountContact.name : ''
                            return (
                              <div
                              ...
                              <div className="flex flex-wrap items-center justify-between gap-2 text-sm font-semibold text-white/80">
                                ...
                                <div>
                                  <label className="block text-xs uppercase tracking-wide text-white/60 mb-2">Titular rezervare</label>
                                  <div className="rounded-lg border border-white/10 bg-white/5 px-3 py-2.5 text-sm text-white/80">
                                    <div className="font-semibold text-white">{displayName || contact.name || '—'}</div>
                                    <div className="text-xs text-white/50">
                                      {hasAccountContact ? 'Preluat automat din cont.' : 'Se completează în datele de contact.'}
                                    </div>
                                  </div>
                                </div>
                              </div>
